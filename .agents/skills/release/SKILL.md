---
name: release
description: Prepare, authorize, publish, troubleshoot, and verify Cursor BYOK desktop GitHub Releases. Use for version bumps, release tags, GitHub Actions release runs, or release-readiness checks.
---

# Desktop release

Release through `.github/workflows/release.yml`. Ship installers only; the app has no in-app updater.
...
3. Confirm neither the intended tag nor Release already exists.
...
5. Validate the workflow YAML and inspect the staged diff. Ensure `.tauri/`, unrelated local files, and unrelated user changes are not staged.
...
5. Verify the Release contains platform installers (NSIS, AppImage/deb, macOS app archives) and no updater manifests such as `latest.json` or `update.json`.
